...
public class MPackage extends X_M_Package
{
	...
	public MPackage (MInOut shipment, MShipper shipper)
	{
		...
		int[] ids = getAllIDs(MShipperLabels.Table_Name, whereClause, shipment.get_TrxName());
		if (ids.length > 0)
			setM_ShipperLabels_ID(ids[0]);

		ids = getAllIDs(MShipperPackaging.Table_Name, whereClause, shipment.get_TrxName());
		if (ids.length > 0)
			setM_ShipperPackaging_ID(ids[0]);

		ids = getAllIDs(MShipperPickupTypes.Table_Name, whereClause, shipment.get_TrxName());
		if (ids.length > 0)
...
//		st.setShippingRespMessage(ShippingRespMessage);
		...
		int[] ids = getAllIDs(MPackageMPS.Table_Name, "M_Package_ID = " + getM_Package_ID(), get_TrxName());
