...
public class MIssue extends X_AD_Issue
{
	...
	private void init(Properties ctx) throws Exception
	{
		...
		setVersion(Adempiere.getI().DATE_VERSION);		//	Code
		setDatabaseInfo(DB.getDatabaseInfo());
		setOperatingSystemInfo(Adempiere.getI().getOSInfo());
		setJavaInfo(Adempiere.getI().getJavaInfo());
		setReleaseTag(Adempiere.getI().getImplementationVersion());
