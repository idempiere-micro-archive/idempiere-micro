...
public class CashFlow  extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		String sqlOpenOrders = "SELECT o.C_Order_ID, o.IsPayScheduleValid, " +
				...
				" ,h.c_conversiontype_id " +
				"FROM C_Order o JOIN C_OrderLine ol ON (o.C_Order_ID=ol.C_Order_ID) " +
				" JOIN c_order_header_v h ON (o.C_Order_ID=h.C_Order_ID) " +
		...
		try
		{
			...
			while (rsOpenOrders.next())
			{
				...
				int conversion_type_id = (rsOpenOrders.getInt("C_ConversionType_ID"));
				...
				if (isPaySchedule) {
					...
					for (MOrderPaySchedule ops : schedule) {
						...
						if (ops.getDueDate().compareTo(p_dateTo) <=0) {
							...
							cforderps.setC_Currency_ID(order.getC_Currency_ID());	
							if (conversion_type_id != 0 ) { cforderps.setC_ConversionType_ID(conversion_type_id); }								
		...
		String sqlActual = "SELECT oi.AD_Org_ID, oi.C_Invoice_ID, oi.C_BPartner_ID, oi.IsSOTrx, oi.DueDate, oi.OpenAmt, oi.C_Campaign_ID, oi.C_Project_ID, oi.C_Activity_ID " +
				", oi.C_Currency_ID, oi.C_ConversionType_ID " +				
				"FROM RV_OpenItem oi " +
		...
		try
		{
			...
			while (rsActual.next())
			{
				...
				cfactual.setC_Currency_ID(rsActual.getInt("C_Currency_ID"));
				cfactual.setC_ConversionType_ID(rsActual.getInt("C_ConversionType_ID"));
				if (!cfactual.save())
