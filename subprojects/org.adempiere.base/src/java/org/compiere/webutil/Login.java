...
public class Login implements ILoginUtility
{
	...
	//	logger.entering("Env", "initTest");
		if (!Adempiere.getI().startupEnvironment(true))
			System.exit (1);
		//  Test Context
	...
	public static boolean isJavaOK (boolean isClient)
	{
		...
		if (isClient)
			JOptionPane.showMessageDialog(null, msg.toString(),
				org.compiere.Adempiere.getI().getName() + " - Java Version Check",
			...
			try
			{
				...
				if (Ini.getIni().isClient())
				{
					if (MSystem.isSwingRememberUserAllowed())
						Ini.getIni().setProperty(Ini.getIni().P_UID, app_user);
					else
						Ini.getIni().setProperty(Ini.getIni().P_UID, "");
					if (Ini.getIni().isPropertyBool(Ini.getIni().P_STORE_PWD) && MSystem.isSwingRememberPasswordAllowed())
						Ini.getIni().setProperty(Ini.getIni().P_PWD, app_pwd);
				}
	...
	//	s_log.fine("loadClients - Role: " + role.toStringX());
		...
		try
		{
			...
			Ini.getIni().setProperty(Ini.getIni().P_ROLE, role.getName());
			//	User Level
	...
	//	s_log.fine("Client: " + client.toStringX() + ", AD_Role_ID=" + AD_Role_ID);
		...
		try
		{
			...
			Ini.getIni().setProperty(Ini.getIni().P_ROLE, rol.getName());
			//	User Level
			Env.setContext(m_ctx, "#User_Level", rs.getString(1));  	//	Format 'SCO'
			//  load Orgs
								   ...
								   INameKeyPair warehouse, java.sql.Timestamp timestamp, String printerName)
	{
		...
		Ini.getIni().setProperty(Ini.getIni().P_ORG, org.getName());

		//  Warehouse Info
		if (warehouse != null)
		{
			Env.setContext(m_ctx, Env.M_WAREHOUSE_ID, warehouse.getKey());
			Ini.getIni().setProperty(Ini.getIni().P_WAREHOUSE, warehouse.getName());
		}
		...
		Ini.getIni().setProperty(Ini.getIni().P_PRINTER, printerName);
	 ...
	 * 	<code>
		...
		Ini.getIni().setProperty(Ini.getIni().P_UID,"SuperUser");
		Ini.getIni().setProperty(Ini.getIni().P_PWD,"System");
		Ini.getIni().setProperty(Ini.getIni().P_ROLE,"GardenAdmin");
		Ini.getIni().setProperty(Ini.getIni().P_CLIENT, "Garden World");
		Ini.getIni().setProperty(Ini.getIni().P_ORG,"HQ");
		Ini.getIni().setProperty(Ini.getIni().P_WAREHOUSE,"HQ");
		Ini.getIni().setProperty(Ini.getIni().P_LANGUAGE,"English");
		Ini.getIni().setProperty(Ini.getIni().P_PRINTER,"MyPrinter");
	...
	{
		//	User Login
		String uid = Ini.getIni().getProperty(Ini.getIni().P_UID);
		...
		//	Language
		String langName = Ini.getIni().getProperty(Ini.getIni().P_LANGUAGE);
		...
		//	Preferences
		String printerName = Ini.getIni().getProperty(Ini.getIni().P_PRINTER);
		if (loginDate == null)
	...
	public INameKeyPair[] getRoles(String app_user, INameKeyPair client) {
		...
		Ini.getIni().setProperty(Ini.getIni().P_CLIENT, client.getName());
		return retValue;
	}   //  getRoles
