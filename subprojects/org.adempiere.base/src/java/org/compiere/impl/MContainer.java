...
public class MContainer extends X_CM_Container
{
		...
		String trxName)
	{
		...
		int[] tableKeys = getAllIDs("CM_CStage_Element",
		...
		if (tableKeys != null && tableKeys.length > 0)
		{
			for (int i = 0; i < tableKeys.length; i++)
			{
				...
				int[] thisContainerElementKeys = getAllIDs("CM_Container_Element", msgx.toString(), trxName);
		...
		tableKeys = getAllIDs("CM_Container_Element",
			msgx.toString(), trxName);
		if (tableKeys != null && tableKeys.length > 0)
		{
			for (int i = 0; i < tableKeys.length; i++)
			{
				...
				int[] thisCStageElementKeys = getAllIDs("CM_CStage_Element", msgx.toString(), trxName);
	...
	{
		StringBuilder msgx = new StringBuilder("CM_CStage_ID=").append(stage.get_ID ());
		int[] tableKeys = getAllIDs(I_CM_CStageTTable.Table_Name,
			msgx.toString(), trxName);
		if (tableKeys != null && tableKeys.length > 0)
		{
			for (int i = 0; i < tableKeys.length; i++)
			{
				...
				int[] thisContainerTTableKeys = getAllIDs(
				...
				copyValues(new X_CM_CStageTTable (project.getCtx (),
	...
	protected MContainerElement[] getAllElements()
	{
		...
		int elements[] = getAllIDs("CM_Container_Element", msgmc.toString(), get_TrxName());
